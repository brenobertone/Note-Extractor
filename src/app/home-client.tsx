...
export default function HomeClient() {
  ...
  // Firebase services (initialized in browser-only effect)
  ...
  useEffect(() => {
    try {
      ...
      // If initFirebase throws here it means something is wrong (e.g., not running in browser)
      // We catch and log so the app doesn't crash during hydration.
      console.warn('Firebase not initialized yet', err);
      setFb(null);
    }
  }, []);

  // Subscribe to Firestore notes only after auth service is available.
  useEffect(() => {
    if (!fb) return;

    const { auth, firestore } = fb;

    let notesUnsub: (() => void) | undefined;
    let authUnsub: (() => void) | undefined;

    try {
      authUnsub = auth.onAuthStateChanged((user: any) => {
        ...
        if (user) {
          ...
          notesUnsub = onSnapshot(notesQuery, (snapshot) => {
            const docs: Note[] = snapshot.docs.map((d: any) => {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    if (!fb) {
      toast({ title: 'Not ready', description: 'Firebase is not initialized yet.', variant: 'destructive' });
      return;
    }

    setIsProcessing(true);

    const { auth, storage, firestore } = fb;
